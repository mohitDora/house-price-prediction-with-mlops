...
class DataProcessor:
    ...
    def load_data(self):
        ...
        try:
            ...
            logger.error(f"Failed to load raw data from {self.raw_data_path}: {e}", exc_info=True)
        ...
        schema = pa.DataFrameSchema(
            {
                ...
                "furnishingstatus": pa.Column(
                    pa.String,
                    checks=pa.Check.isin(["furnished", "semi-furnished", "unfurnished"]),
                ),
            }
        )
        try:
            ...
            logger.error(f"Raw data schema validation failed: {err.failure_cases}", exc_info=True)
            raise MyException(f"Raw data schema validation failed: {err.failure_cases}", sys)
        except Exception as e:
            logger.error(
                f"An unexpected error occurred during raw data validation: {e}",
                exc_info=True,
            )
            raise MyException(f"An unexpected error occurred during raw data validation: {e}", sys)
    ...
    def save_processed_artifacts(self, X_processed, y, preprocessor):
        ...
        try:
            processed_data = {"X": X_processed, "y": y.values}
...
if __name__ == "__main__":
    ...
    try:
        ...
        intermediate_df_for_validation = data_processor._convert_yes_no_to_binary(
            raw_df_from_csv.copy(),
            ["mainroad", "guestroom", "basement", "hotwaterheating", "airconditioning", "prefarea"],
        )
